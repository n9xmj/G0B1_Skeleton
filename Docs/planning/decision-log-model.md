# Decision-log planning model (G0B1_Skeleton)

**Purpose:** living plan documents for multi-session design work — feature specs moving
toward implementation (the portable-API restructuring, nvmparams, and whatever follows).
...
neither SwitchTester nor Skeleton has them.
...
Plans live at `Docs/planning/<topic>-plan.md` — e.g. `logging-api-plan.md`.

### 1. Header block

Title, one-paragraph feature description, code home, link to the parent spec
(`Docs/planning/portable-apis-strategy.md`), status (PLANNING · IN PROGRESS · IMPLEMENTING · DONE),
...
6. After decisions land, **sync `portable-apis-strategy.md` from the plan** — the plan is
   the negotiation log, the strategy doc is the contract. Don't maintain detail in both.
...
| Doc | Role |
|-----|------|
| [`portable-apis-strategy.md`](portable-apis-strategy.md) | Conventions contract — the standing rules once decisions land |
| [`improvements-backlog.md`](improvements-backlog.md) | Captured-but-unbuilt ideas |
| [`logging-api-plan.md`](logging-api-plan.md) | First plan in this repo; worked example of the model |
